...
(function($, undefined) {
	...
	$.expr[':'].findday = $.expr.createPseudo ? $.expr.createPseudo(function(
			text) {
		return function(elt) {
	...
	$.expr[':'].findweek = $.expr.createPseudo ? $.expr.createPseudo(function(
			text) {
		return function(elt) {
	...
	$.expr[':'].findmonth = $.expr.createPseudo ? $.expr.createPseudo(function(
			text) {
		return function(elt) {
	...
	Date.prototype.getDayOfYear = function() {
		...
		return (Date.UTC(year, this.getMonth(), this.getDate()) - Date.UTC(
				year, 0, 0))
				/ UTC_DAY_IN_MS;
	};
	...
	var firstDay = 1; // ISO week starts with Monday (1); use Sunday (0) for,
						// e.g., North America
	var weekOneDate = 4; // ISO week one always contains 4 Jan; use 1 Jan
	...
	Date.prototype.getWeekOfYear = function() {
		var year = this.getFullYear(), month = this.getMonth(), date = this
				.getDate(), day = this.getDay();
	...
	function ktkGetNextDate(currentDate, scaleStep) {
		for (var minIncrements = 1;; minIncrements++) {
			...
			nextDate.setHours(currentDate.getHours() + scaleStep
					* minIncrements);
			...
			// If code reaches here, it's because current didn't really
			// increment (invalid local time) because of daylight-saving
			// adjustments
			// => retry adding 2, 3, 4 hours, and so on (until nextDate >
			// current)
		}
	}
	...
	$.fn.gantt = function(options) {
		...
		var core = {
			...
			// Normalizes for old browsers (NOTE: doesn't work when element is
			// outside viewport)
			...
			init : function(element) {
				...
				var msPerUnit = 4.35e7 * daysPerUnit;
				var totalUnitsNow = Math.ceil(msInCurrentRange / msPerUnit) + 1;
				var totalUnitsNeeded = Math.ceil($(element).width()
						/ tools.getCellSize());
				var unitsToAdd = totalUnitsNeeded - totalUnitsNow;
				if (unitsToAdd > 0) {
					element.dateEnd.setDate(element.dateEnd.getDate()
							+ (unitsToAdd * daysPerUnit));
				}
			...
			leftPanel : function(element) {
				...
				var ganttLeftPanel = $('<div class="leftPanel"/>').append(
						$('<div class="row spacer"/>').css("height",
								tools.getCellSize() * element.headerRows).css(
								"width", "100%"));
						...
						.each(
								...
								function(i, entry) {
									if (i >= element.pageNum
										...
										var dataId = ('id' in entry) ? '" data-id="'
												+ entry.id
												: '';
										entries
												.push('<div class="row name row'
														+ i
														+ (entry.desc ? ''
																: (' fn-wide ' + dataId))
														+ '" id="rowheader'
			...
			dataPanel : function(element, width) {
				var dataPanel = $('<div class="dataPanel" style="width: '
						+ width + 'px;"/>');

				// Handle mousewheel events for scrolling the data panel
				var wheel = 'onwheel' in element ? 'wheel'
				...
				dataPanel.click(function(e) {
					...
					/*
					 * Adjust, so get middle of elm corrY -=
					 * Math.floor(tools.getCellSize() / 2);
					 */

					// Find column where click occurred
					var col = core.elementFromPoint(e.pageX,
							datapanel.offset().top + corrY);
			...
			rightPanel : function(element, leftPanel /* <- never used? */) {
				...
				// unused: was someone planning to allow styles to stretch to
				...
				case "hours":
					...
					for (i = 0, len = range.length; i < len; i++) {
						...
						if (rgetDay !== getDay) {
							dayClass = (today - day === 0) ? "today" : tools
									.isHoliday(day.getTime()) ? "holiday"
									: dowClass[getDay];

							dayArr.push('<div class="row date ' + dayClass
									+ '" ' + 'style="width: '
									+ tools.getCellSize() * hoursInDay
									+ 'px;">' + '<div class="fn-label">'
									+ day.getDate() + '</div></div>');
							dowArr.push('<div class="row day ' + dayClass
									+ '" ' + 'style="width: '
									+ tools.getCellSize() * hoursInDay
									+ 'px;">' + '<div class="fn-label">'
						...
						horArr.push('<div class="row day ' + dayClass
								+ '" id="dh-' + rday.getTime()
								+ '" data-offset="' + i * tools.getCellSize()
								+ '" data-repdate="'
								+ rday.getRepDate(settings.scale)
								+ '"><div class="fn-label">' + rday.getHours()
								+ '</div></div>');
					}
					...
					dataPanel.append($row.clone().html(yearArr.join("")), $row
							.clone().html(monthArr.join("")), $row.clone()
							.html(dayArr.join("")), $row.clone().html(
							dowArr.join("")), $row.clone()
							.html(horArr.join("")));
					break;
				...
				case "weeks":
					...
					for (i = 0, len = range.length; i < len; i++) {
						...
						if (week > (week = rday.getWeekOfYear())) {
							...
							diff = rday.getDate() - 1;
							// offset one month (December) if week starts in
							// last year
						...
						dayArr.push('<div class="row day wd"' + ' id="'
								+ rday.getWeekId() + '" data-offset="' + i
								* tools.getCellSize() + '" data-repdate="'
								+ rday.getRepDate(settings.scale) + '">'
								+ '<div class="fn-label">' + week
								+ '</div></div>');
					}
				...
				case 'months':
					...
					for (i = 0, len = range.length; i < len; i++) {
						...
						monthArr.push('<div class="row day wd" id="dh-'
								+ tools.genId(rday) + '" data-offset="' + i
								* tools.getCellSize() + '" data-repdate="'
								+ rday.getRepDate(settings.scale) + '">'
								+ (1 + rday.getMonth()) + '</div>');
					}
				...
				default:
					...
					for (i = 0, len = range.length; i < len; i++) {
						...
						dowArr.push('<div class="row day ' + dayClass + '"'
								...
								+ '" data-repdate="'
								+ rday.getRepDate(settings.scale) + '">'
								+ '<div class="fn-label">' + settings.dow[day]
								+ '</div></div>');
					} // for
			...
			// **Progress Bar**
			// Return an element representing a progress of position within the
			// entire chart
			createProgressBar : function(days, label, desc, classNames, dataObj) {
				...
				var bar = $(
						'<div class="bar"><div class="fn-label">' + label
								+ '</div></div>').css({
			...
			markNow : function(element) {
				...
				case "weeks":
					...
					break;
				case "months":
					$(element).find(':findmonth("' + cd + '")').removeClass(
							'wd').addClass('today');
					break;
				case "days":
					/* falls through */
			...
			fillData : function(element, datapanel, leftpanel /*
				...
				var invertColor = function(colStr) {
					try {
						...
						var gray = Math
								.round((255 - (0.299 * R + 0.587 * G + 0.114 * B)) * 0.9);
			...
			navigateTo : function(element, val) {
				...
				case "now":
					...
					val = $dataPanel.find(".today").offset().left
			...
			zoomInOut : function(element, val) {
				core.waitToggle(element, function() {
					...
					if (settings.useCookie) {
						$.cookie(settings.cookieKey + "CurrentScale",
								settings.scale);
						// reset scrollPos
			...
			wheelScroll : function(element, e) {
				...
				var delta = ('detail' in e ? e.detail
						: 'wheelDelta' in e.originalEvent ? -1 / 120
								* e.originalEvent.wheelDelta
								: e.originalEvent.deltaY ? e.originalEvent.deltaY
										/ Math.abs(e.originalEvent.deltaY)
										: e.originalEvent.detail);

				// simpler normalization, ignoring per-device/browser/platform
				// acceleration & semantic variations
				// var delta = e.detail || - (e = e.originalEvent).wheelData ||
				// e.deltaY /* || e.deltaX */ || e.detail;
			...
			scrollPanel : function(element, delta) {
				...
				if (_panelMargin > 0) {
					...
					$(element).find(".fn-gantt .dataPanel").css("margin-left",
							element.scrollNavigation.panelMargin);
			...
			repositionLabel : function(element) {
				setTimeout(function() {
					...
					if (settings.useCookie) {
						$.cookie(settings.cookieKey + "ScrollPos", $dataPanel
		...
		var tools = {
			...
			parseTimeRange : function(from, to, scaleStep) {
				...
				for (;;) {
					...
					dayStartTime.setHours(Math.floor((current.getHours())
							/ scaleStep)
							* scaleStep);

					if (ret[i] && dayStartTime.getDay() !== ret[i].getDay()) {
						// If mark-cursor jumped to next day, make sure it
						// starts at 0 hours
						dayStartTime.setHours(0);
					}
					ret[i] = dayStartTime;

					// Note that we use ">" because we want to include the
					// end-time point.
					if (current > to) {
						break;
					}
					...
					/*
					 * BUG-2: current is moved backwards producing a dead-lock!
					 * (crashes chrome/IE/firefox) SEE:
					 * https://github.com/taitems/jQuery.Gantt/issues/62 if
					 * (current.getDay() !== ret[i].getDay()) {
					 * current.setHours(0); }
					 */
			...
			dateDeserialize : function(date) {
				if (typeof date === "string") {
					...
					date = $.isNumeric(date) ? parseInt(date, 10) : $
							.trim(date);
				}
			...
			// normalizes an array of dates into a map of start-of-day
			// millisecond values
			...
			// Returns true when the given date appears in the array of
			...
			isHoliday : (function() { // IIFE
				...
				var holidays = false;
				// returns the function that will be used to check for
				// holidayness of a given date
				return function(date) {
					if (!holidays) {
						holidays = tools._datesToDays(settings.holidays);
					}
					return !!holidays[
					// assumes numeric dates are already normalized to
					// start-of-day
					$.isNumeric(date) ? date : (new Date(date.getFullYear(),
			...
			getProgressBarMargin : function() {
				if (typeof tools._getProgressBarMargin === "undefined") {
					...
					$("body").append(measure);
					tools._getProgressBarMargin = parseInt(bar.css(
							"margin-left").replace("px", ""), 10);
					tools._getProgressBarMargin += parseInt(bar.css(
